...
module.exports = async (event, say) => {
  ...
  const user = await User.findOneAndUpdate({ slackId }, { slackId }, { upsert: true, new: true });
  const msgs = [];
  for (const iso of dates) {
    ...
    const exist = await Leave.findOne({ userId: user._id, date: d.toDate(), status: 'planned' });
    if (exist) msgs.push(`Already planned ${iso}`);
    else { await Leave.create({ userId: user._id, date: d.toDate() }); msgs.push(`Added ${iso}`); }
  }
  say(msgs.join('\n'));
};